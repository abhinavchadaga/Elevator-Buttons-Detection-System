...
def validation_loop(model: torch.nn, dataloader: torch.utils.data.DataLoader) -> dict:
    ...
    comm.synchronize()
    log_first_n(logging.INFO, f"VALIDATION_LOSS: {val_loss:.5f}")
...
def main():
    ...
    max_iters, eval_period = epochs_to_iters(
        f"{dataset_name}_train", args.epochs, args.batch_size, args.eval_every_n_epochs
    )

    log_every_n_seconds(logging.INFO, f"Successfully loaded {dataset_name} dataset")
    log_every_n_seconds(
        logging.INFO, f"training for {max_iters} iters, eval every {eval_period}"
    )
    ...
    test_loader = trainer.build_test_loader(cfg, cfg.DATASETS.TEST[1])
